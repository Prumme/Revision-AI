...
// Pour le quiz step-by-step
const currentStep = ref(0);
const userAnswers = ref<Record<number, number[]>>({}); // {questionIndex: [selectedAnswerIndexes]}
...
const quizTabs = [
  { key: "config", label: "Configuration du quiz" },
  { key: "quiz", label: "Faire le quiz" },
];
...
<template>
  ...
  <!-- Tabs (même style que SubscriptionView) -->
  ...
  <section v-if="quiz && activeTab === 'quiz'" class="max-w-2xl mx-auto w-full">
    <div v-if="!quizFinished">
      <div class="mb-6">
        <div class="font-bold text-lg mb-2">Question {{ currentStep + 1 }} / {{ quiz.questions.length }}</div>
        <QuestionDraggable
          :questions="[quiz.questions[currentStep]]"
          :showAllAnswers="quizFinished"
      ...
      <Button
        ...
        @click="nextStep"
      >
        {{ currentStep === quiz.questions.length - 1 ? 'Voir le résultat' : 'Question suivante' }}
      </Button>
    </div>
    <div v-else class="text-center py-10">
      ...
      <Button class="mt-4" @click="() => { currentStep = 0; quizFinished = false; userAnswers = {}; quizScore = 0; }">
        Recommencer le quiz
      </Button>
    </div>
  </section>
