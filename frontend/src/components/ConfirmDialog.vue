<template>
  <Transition name="dialog">
    ...
    >
      <div
        class="bg-white border-2 border-black rounded-2xl shadow-[0_8px_0_#000] w-full max-w-xl min-w-[350px] px-8 py-7 flex flex-col gap-6 font-outfit animate-[pop-in_0.25s_cubic-bezier(.68,-0.55,.27,1.55)]"
        @click.stop
      >
        <div class="border-b border-gray-200 pb-3">
          <h2 class="text-2xl font-extrabold text-zinc-900 m-0">{{ dialogStore.options.title }}</h2>
        </div>
        <div>
          <p class="text-base text-zinc-800 leading-relaxed">{{ dialogStore.options.message }}</p>
        </div>
